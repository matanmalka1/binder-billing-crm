...
def upgrade() -> None:
    if is_postgresql():
        ...
        op.execute("""
            ...
            END $$;
        """)
        op.execute("ALTER TABLE permanent_documents ALTER COLUMN status DROP DEFAULT;")
        op.execute("""
        ...
        """)
        op.execute("ALTER TABLE permanent_documents ALTER COLUMN status SET DEFAULT 'pending'::documentstatus;")
...
def downgrade() -> None:
    ...
    if is_postgresql():
        ...
        """)
        op.execute("ALTER TABLE permanent_documents ALTER COLUMN status SET DEFAULT 'pending';")
