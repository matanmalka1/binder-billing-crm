...
Revises: 
Create Date: 2026-05-17 11:17:32.180021

Run:
- Upgrade:   APP_ENV=<env> ENV_FILE=<env_file> python3 -m alembic upgrade 0001_initial
- Downgrade: APP_ENV=<env> ENV_FILE=<env_file> python3 -m alembic downgrade base

Notes:
- Fresh baseline migration generated from the current SQLAlchemy models.
- Previous incremental migration history was intentionally reset.
- Intended for an empty database/schema. Development DB can be reset and upgraded to this revision.
- Partial indexes use sa.text(...) for PostgreSQL and SQLite where clauses.
"""
