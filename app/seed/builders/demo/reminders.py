...
def create_reminders(db, rng: Random, businesses, binders, charges, deadlines) -> None:
    ...
    for business in businesses:
        ...
        idle_binders = [
            b for b in business_binders if b.status != BinderStatus.RETURNED
        ]
        if idle_binders:
            binder = rng.choice(idle_binders)
            days_idle = (
                max(14, (today - binder.period_start).days)
                if binder.period_start
                else 14
            )
            _add_unique(
                db,
        ...
        client_deadlines = [
            dl
            for dl in deadlines_by_client.get(business.client_id, [])
            ...
            if target_date >= today:
                _add_unique(
                    ...
                    Reminder(
                        ...
                        - timedelta(days=rng.randint(0, 120)),
                    ),
                )
