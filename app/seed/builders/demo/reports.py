...
VALID_TRANSITIONS: dict[AnnualReportStatus, set[AnnualReportStatus]] = {
    ...
    AnnualReportStatus.AMENDED: {AnnualReportStatus.IN_PREPARATION, AnnualReportStatus.SUBMITTED},
    ...
    AnnualReportStatus.ASSESSMENT_ISSUED: {
        ...
        AnnualReportStatus.PENDING_CLIENT, AnnualReportStatus.IN_PREPARATION,
        AnnualReportStatus.DOCS_COMPLETE,
    },
    AnnualReportStatus.OBJECTION_FILED: {AnnualReportStatus.CLOSED, AnnualReportStatus.DOCS_COMPLETE},
...
def _build_annex_payload(rng: Random, report: AnnualReport, schedule: AnnualReportSchedule) -> dict[str, Any]:
    if schedule == AnnualReportSchedule.SCHEDULE_A:
        ...
        return {"gross_income": _as_plain_decimal(gross), "cost_of_goods": _as_plain_decimal(cogs), "gross_profit": _as_plain_decimal(gp), "operating_expenses": _as_plain_decimal(opex), "net_income": _as_plain_decimal(gp - opex)}
    if schedule == AnnualReportSchedule.SCHEDULE_B:
        ...
        return {"property_address": _property_address(rng), "rental_income": _as_plain_decimal(ri), "depreciation_claimed": _as_plain_decimal(dep), "maintenance_expenses": _as_plain_decimal(maint), "net_rental_income": _as_plain_decimal(ri - maint - dep)}
    if schedule == AnnualReportSchedule.FORM_1399:
        pp = _random_decimal(rng, 25_000, 220_000)
        sp = pp + _random_decimal(rng, 5_000, 90_000)
        return {"asset_description": rng.choice(["מכירת דירה להשקעה", "מכירת ציוד עסקי"]), "purchase_date": f"{report.tax_year - rng.randint(1, 6)}-{rng.randint(1, 12):02d}-{rng.randint(1, 28):02d}", "sale_date": f"{report.tax_year}-{rng.randint(1, 12):02d}-{rng.randint(1, 28):02d}", "purchase_price": _as_plain_decimal(pp), "sale_price": _as_plain_decimal(sp), "capital_gain": _as_plain_decimal(sp - pp), "tax_rate": _as_plain_decimal(_random_decimal(rng, 20, 30))}
    if schedule == AnnualReportSchedule.SCHEDULE_DALET:
        ...
        return {"country": rng.choice(COUNTRIES), "income_type": rng.choice(["משכורת", "דיבידנד", "ריבית"]), "gross_income": _as_plain_decimal(gi), "foreign_tax_paid": _as_plain_decimal(ft), "net_income": _as_plain_decimal(gi - ft)}
    if schedule == AnnualReportSchedule.FORM_1342:
        ...
        return {"asset_description": rng.choice(["מחשב נייד", "ריהוט משרדי"]), "asset_cost": _as_plain_decimal(ac), "depreciation_rate": _as_plain_decimal(_random_decimal(rng, 7, 20)), "accumulated_depreciation": _as_plain_decimal(acc), "annual_depreciation": _as_plain_decimal(ad), "book_value": _as_plain_decimal(max(ac - acc, Decimal("0.00")))}
    if schedule == AnnualReportSchedule.SCHEDULE_GIMMEL:
        pp = _random_decimal(rng, 8_000, 70_000)
        sp = pp + rng.choice([_random_decimal(rng, 1_500, 20_000), -_random_decimal(rng, 500, 12_000)])
        return {"security_name": rng.choice(SECURITIES), "quantity": _as_plain_decimal(_random_decimal(rng, 10, 600)), "purchase_price": _as_plain_decimal(pp), "sale_price": _as_plain_decimal(sp), "gain_loss": _as_plain_decimal(sp - pp)}
    if schedule == AnnualReportSchedule.FORM_150:
        ...
        return {"country": rng.choice(COUNTRIES), "income_description": rng.choice(["הכנסות ייעוץ", "דיבידנד"]), "gross_amount": _as_plain_decimal(ga), "withholding_tax": _as_plain_decimal(wh), "treaty_rate": _as_plain_decimal(_random_decimal(rng, 5, 25))}
    if schedule == AnnualReportSchedule.FORM_1343:
        return {"bank_name": rng.choice(["בנק הפועלים", "בנק לאומי", "מזרחי טפחות"]), "account_number": f"{rng.randint(10000, 99999)}-{rng.randint(1000000, 9999999)}", "interest_income": _as_plain_decimal(_random_decimal(rng, 300, 8_000)), "dividend_income": _as_plain_decimal(_random_decimal(rng, 0, 12_000)), "withholding_tax": _as_plain_decimal(_random_decimal(rng, 0, 2_500))}
    return {}
...
def _status_path_to(target: AnnualReportStatus) -> list[AnnualReportStatus]:
    if target in (AnnualReportStatus.AMENDED, AnnualReportStatus.CANCELED, AnnualReportStatus.NOT_STARTED):
        return [target]
        ...
        for year in years:
            ...
            submission_method = (
                ...
                if status in (
                    ...
                    AnnualReportStatus.ASSESSMENT_ISSUED, AnnualReportStatus.OBJECTION_FILED,
                    AnnualReportStatus.CLOSED,
                )
                else None
            )
            if deadline_type == DeadlineType.STANDARD:
                filing_deadline = standard_deadline(year, client_type=client_type_for_report, submission_method=submission_method)
            elif deadline_type == DeadlineType.EXTENDED:
                filing_deadline = extended_deadline(year)
            else:
                filing_deadline = None

            extension_reason = rng.choice(list(ExtensionReason)) if deadline_type == DeadlineType.EXTENDED else None
            ...
            if status in (
                ...
                submitted_at = min(datetime.now(UTC), created_at + timedelta(days=rng.randint(1, 180)))
            ...
            report = AnnualReport(
                ...
                custom_deadline_note="המועד עודכן בהתאם לאישור ארכה" if deadline_type == DeadlineType.CUSTOM else None,
...
def create_annual_report_details(db, rng: Random, reports) -> None:
    for report in reports:
        ...
        from app.annual_reports.models.annual_report_detail import AnnualReportDetail as ARD
        if db.query(ARD).filter(ARD.report_id == report.id).first():
            continue
        client_approved_at = None
        if report.status in (
            AnnualReportStatus.SUBMITTED, AnnualReportStatus.ASSESSMENT_ISSUED,
            AnnualReportStatus.ACCEPTED, AnnualReportStatus.OBJECTION_FILED, AnnualReportStatus.CLOSED,
        ):
        ...
        db.add(AnnualReportDetail(
            ...
            internal_notes=rng.choice([None, "ממתין לאישור לקוח", "לעדכן נתוני שכר", "לבדוק קלטי מע\"מ"]),
            amendment_reason=(
                rng.choice(["תיקון לפי מסמכים מעודכנים", "תיקון בעקבות שומת מס"])
                if report.status == AnnualReportStatus.AMENDED else None
            ),
...
def create_annual_report_schedule_entries(db, rng: Random, reports, users=None) -> None:
    ...
    for report in reports:
        for schedule in list(AnnualReportSchedule):
            if db.query(AnnualReportScheduleEntry).filter(
            ...
            db.add(AnnualReportScheduleEntry(
...
def create_annual_report_income_lines(db, rng: Random, reports) -> None:
    for report in reports:
        ...
        if report.client_type in (ClientTypeForReport.SELF_EMPLOYED, ClientTypeForReport.CORPORATION):
            source_candidates.extend([IncomeSourceType.BUSINESS, IncomeSourceType.INTEREST, IncomeSourceType.DIVIDENDS])
        if report.has_rental_income:
        ...
        for source_type in rng.sample(unique_sources, k=min(len(unique_sources), rng.randint(1, 3))):
            db.add(AnnualReportIncomeLine(
                annual_report_id=report.id,
...
def create_annual_report_expense_lines(db, rng: Random, reports, seeded_documents: Iterable | None = None) -> None:
    base_categories = [
    ...
    for report in reports:
        ...
        if report.client_type in (ClientTypeForReport.SELF_EMPLOYED, ClientTypeForReport.CORPORATION):
            category_candidates.extend([
                ExpenseCategoryType.OFFICE_RENT, ExpenseCategoryType.COMMUNICATION,
                ExpenseCategoryType.VEHICLE, ExpenseCategoryType.MARKETING,
                ExpenseCategoryType.TRAVEL, ExpenseCategoryType.TRAINING,
            ])
        if report.has_depreciation:
        ...
        for category in rng.sample(unique_categories, k=min(len(unique_categories), rng.randint(1, 4))):
            ...
            db.add(AnnualReportExpenseLine(
                annual_report_id=report.id,
                category=category,
...
def create_annual_report_credit_points(db, rng: Random, reports) -> None:
    for report in reports:
        ...
        for reason in reason_candidates[:rng.randint(1, min(3, len(reason_candidates)))]:
            ...
            db.add(AnnualReportCreditPoint(
                annual_report_id=report.id,
                reason=reason,
                points=points,
...
def create_annual_report_status_history(db, rng: Random, reports, users) -> None:
    ...
    for report in reports:
        ...
        for status in history_statuses:
            ...
            db.add(AnnualReportStatusHistory(
                annual_report_id=report.id,
                from_status=previous,
                to_status=status,
                changed_by=actor_id,
                note=rng.choice(["היסטוריית סטטוסים שנוצרה אוטומטית", "עודכן לאחר בדיקת מסמכים", "הועבר לשלב הבא"]),
