...
class ClientLifecycleService:
    ...
    def restore_client(self, client_id: int, actor_id: int):
        ...
        legal_entity = getattr(client, "legal_entity", None)
        if legal_entity and self.record_repo.get_active_by_id_number(legal_entity.id_number):
            raise ConflictError(
                CLIENT_ID_NUMBER_ACTIVE_EXISTS.format(id_number=legal_entity.id_number),
                "CLIENT.CONFLICT",
            )
        restored_record = self.record_repo.restore(client_id, restored_by=actor_id)
        restored = self.client_repo.get_by_id(restored_record.id) if restored_record else None
        if not restored:
            raise NotFoundError(CLIENT_NOT_FOUND.format(client_id=client_id), "CLIENT.NOT_FOUND")
        self._audit.append(
