"""Annual report PDF export — working draft (טיוטה לעיון)."""
...
_CLIENT_TYPE_LABELS: dict[str, str] = {
...
class AnnualReportPdfService:
    ...
    def generate(self, report_id: int) -> tuple[bytes, int]:
        ...
        return _build_pdf(report, client_name, summary, tax, detail), report.tax_year


def _build_pdf(report, client_name: str, summary, tax, detail) -> bytes:  # noqa: ANN001
    ...
    def make_table(rows: list[list]) -> Table:
        ...
        for i, row in enumerate(rows):
            ...
            # RTL layout: value (amount) on left, label on right
        ...
        style_cmds = [
            ...
            # All cells right-aligned (RTL document)
            ("ALIGN",         (0, 0), (-1, -1),     "RIGHT"),
        ]
        # Alternating rows (skip header row 0 and total row n-1)
    ...
    # Section 6 — סיכום
    refund = detail.tax_refund_amount if detail else None
    due    = detail.tax_due_amount    if detail else None
