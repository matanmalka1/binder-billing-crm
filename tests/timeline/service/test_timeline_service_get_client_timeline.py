...
def test_append_status_change_events_handles_enum_values(test_db, monkeypatch):
    ...
    logs = [
        ...
        SimpleNamespace(old_status=None, new_status=BinderStatus.IN_OFFICE, changed_at=datetime(2026, 2, 1, 9, 0)),
        # same-status expressed as Enum — must be skipped
        SimpleNamespace(old_status=BinderStatus.IN_OFFICE, new_status=BinderStatus.IN_OFFICE, changed_at=datetime(2026, 2, 2, 9, 0)),
        # meaningful transition — must be kept
        SimpleNamespace(old_status=BinderStatus.IN_OFFICE, new_status=BinderStatus.READY_FOR_PICKUP, changed_at=datetime(2026, 2, 3, 9, 0)),
    ]
