...
def test_get_status_card_default_year_and_aggregations(test_db):
    ...
    service._vat_repo.list_by_business = lambda _business_id: [
    ...
    assert card.vat.net_vat_total == Decimal("15.00")
    assert card.vat.periods_filed == 1
    assert card.vat.periods_total == 2
    assert card.vat.latest_period == "2026-02"
