...
def test_unpaid_charge_subject_removes_business_and_period_fragments():
    ...
    unpaid = unpaid_charge_attention_item(
        charge, business, "מאיה כץ", date(2026, 4, 29)
    )
    ...
    assert (
        unpaid["charge_subject"]
        == 'חשבונית #1048 · טיפול בדיווח מע"מ תקופתי · תקופה 01/2025'
    )
...
def test_unpaid_charge_subject_removes_client_name_fragment():
    ...
    unpaid = unpaid_charge_attention_item(
        charge, business, "אמיר ביטון", date(2026, 4, 29)
    )
...
def test_unpaid_charge_amount_does_not_render_nan():
    ...
    unpaid = unpaid_charge_attention_item(
        charge, business, "לקוח בדיקה", date(2026, 4, 29)
    )
