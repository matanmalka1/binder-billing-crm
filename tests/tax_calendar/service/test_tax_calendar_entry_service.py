...
def _seed_all_rules(db) -> dict[DeadlineRuleType, DeadlineRule]:
    return {
        DeadlineRuleType.VAT_MONTHLY:       _seed_rule(db, rule_type=DeadlineRuleType.VAT_MONTHLY,       due_day_of_month=15, offset_months=1),
        DeadlineRuleType.VAT_BIMONTHLY:     _seed_rule(db, rule_type=DeadlineRuleType.VAT_BIMONTHLY,     due_day_of_month=15, offset_months=2),
        DeadlineRuleType.ADVANCE_MONTHLY:   _seed_rule(db, rule_type=DeadlineRuleType.ADVANCE_MONTHLY,   due_day_of_month=15, offset_months=1),
        DeadlineRuleType.ADVANCE_BIMONTHLY: _seed_rule(db, rule_type=DeadlineRuleType.ADVANCE_BIMONTHLY, due_day_of_month=15, offset_months=2),
        DeadlineRuleType.ANNUAL_REPORT:    _seed_rule(db, rule_type=DeadlineRuleType.ANNUAL_REPORT,     due_day_of_month=31, offset_months=4),
    }


def _count_entries(db, *, obligation_type: ObligationType, period_months_count: int | None = None, tax_year: int | None = None) -> int:
...
def test_get_or_create_periodic_is_idempotent(test_db):
    ...
    e1, c1 = get_or_create_entry(
        test_db, obligation_type=ObligationType.VAT,
        period="2026-04", period_months_count=1, tax_year=2026,
        deadline_rule_id=rule.id, due_date=date(2026, 5, 15),
    )
    test_db.commit()
...
def test_get_or_create_annual_is_idempotent(test_db):
    rule = _seed_rule(test_db, rule_type=DeadlineRuleType.ANNUAL_REPORT, due_day_of_month=31, offset_months=4)
    e1, c1 = get_or_create_entry(
        test_db, obligation_type=ObligationType.ANNUAL_REPORT,
        period=None, period_months_count=None, tax_year=2025,
        deadline_rule_id=rule.id, due_date=date(2026, 5, 31),
    )
    test_db.commit()
    e2, c2 = get_or_create_entry(
        test_db, obligation_type=ObligationType.ANNUAL_REPORT,
...
def test_vat_monthly_creates_12_entries_per_year(test_db):
    ...
    test_db.commit()
    assert _count_entries(test_db, obligation_type=ObligationType.VAT, period_months_count=1, tax_year=2026) == 12
...
def test_vat_bimonthly_creates_6_entries_per_year(test_db):
    ...
    generate_for_year(test_db, 2026)
    test_db.commit()
    assert _count_entries(test_db, obligation_type=ObligationType.VAT, period_months_count=2, tax_year=2026) == 6
...
def test_advance_monthly_creates_12_entries_per_year(test_db):
    ...
    generate_for_year(test_db, 2026)
    test_db.commit()
    assert _count_entries(test_db, obligation_type=ObligationType.ADVANCE_PAYMENT, period_months_count=1, tax_year=2026) == 12
...
def test_advance_bimonthly_creates_6_entries_per_year(test_db):
    ...
    generate_for_year(test_db, 2026)
    test_db.commit()
    assert _count_entries(test_db, obligation_type=ObligationType.ADVANCE_PAYMENT, period_months_count=2, tax_year=2026) == 6
...
def test_annual_creates_one_entry_per_tax_year(test_db):
    ...
    generate_for_year(test_db, 2026)
    test_db.commit()
    annual = test_db.query(TaxCalendarEntry).filter(
...
def test_rule_outside_effective_window_is_treated_as_missing(test_db):
    ...
    _seed_rule(test_db, rule_type=DeadlineRuleType.VAT_MONTHLY,
               due_day_of_month=15, offset_months=1,
