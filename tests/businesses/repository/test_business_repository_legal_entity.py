...
def client_row(test_db):
    c = Client(full_name="Test Client", id_number="C-001")
    test_db.add(c)
    test_db.flush()
    return c
...
def _make_business(db, client_id, legal_entity_id, *, status=BusinessStatus.ACTIVE, deleted=False):
    global _biz_counter
    _biz_counter += 1
    b = Business(
        client_id=client_id,
...
def test_exists_for_legal_entity_true(test_db, repo, client_row, legal_entity):
    _make_business(test_db, client_row.id, legal_entity.id)
    assert repo.exists_for_legal_entity(legal_entity.id) is True


def test_exists_for_legal_entity_false_when_deleted(test_db, repo, client_row, legal_entity):
    _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
...
def test_all_non_deleted_are_closed_for_legal_entity_true(test_db, repo, client_row, legal_entity):
    _make_business(test_db, client_row.id, legal_entity.id, status=BusinessStatus.CLOSED)
    assert repo.all_non_deleted_are_closed_for_legal_entity(legal_entity.id) is True


def test_all_non_deleted_are_closed_for_legal_entity_false_when_active(test_db, repo, client_row, legal_entity):
    _make_business(test_db, client_row.id, legal_entity.id, status=BusinessStatus.ACTIVE)
...
def test_get_ids_by_legal_entity_returns_ids(test_db, repo, client_row, legal_entity):
    b1 = _make_business(test_db, client_row.id, legal_entity.id)
    b2 = _make_business(test_db, client_row.id, legal_entity.id)
    _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
...
def test_list_by_legal_entity_returns_active(test_db, repo, client_row, legal_entity):
    b = _make_business(test_db, client_row.id, legal_entity.id)
    _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
...
def test_count_by_legal_entity(test_db, repo, client_row, legal_entity):
    _make_business(test_db, client_row.id, legal_entity.id)
    _make_business(test_db, client_row.id, legal_entity.id)
    _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
...
def test_list_by_legal_entity_including_deleted(test_db, repo, client_row, legal_entity):
    b_active = _make_business(test_db, client_row.id, legal_entity.id)
    b_deleted = _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
...
def test_list_by_legal_entity_ids_returns_matching(test_db, repo, client_row, legal_entity):
    ...
    b1 = _make_business(test_db, client_row.id, legal_entity.id)
    b2 = _make_business(test_db, client_row.id, le2.id)
    _make_business(test_db, client_row.id, legal_entity.id, deleted=True)
